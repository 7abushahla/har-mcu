// M3 HAR INT8 (LIVE IMU + Serial Control) — Nano 33 BLE + optional TinyML shield button.
// Session → windowing matches offline pipeline.
//
// Two control surfaces (interchangeable):
//   1. Physical button (TinyML shield D13) — short click = START/STOP toggle, long-hold = average.
//   2. Serial commands (use Arduino Serial Monitor or deploy/m3_nano_int8_serial_imu/serial_controller.py):
//        start  | stop  | toggle    → same as a button click (toggles recording state)
//        avg                        → same as long-hold (mean of buffered trials + confusion matrix)
//        gt <name>                  → set ground-truth class
//        <name>                     → set ground-truth class (bare form, for back-compat)
//        ping                       → desktop heartbeat; firmware prints [PONG] + [STATE]
//        help                       → show all commands
//
// Machine-readable lines for the desktop controller:
//   [STATE] recording=0|1            — emitted on each toggle and on PONG
//   [PONG] m3_nano_int8_serial_imu  — heartbeat reply
//   trial=N win=k/M ... pred=NAME conf=X.X%   — per-window prediction (already emitted)
//
// **Serial (typing class name)**: set ground truth before START, then each sliding window counts
// in a confusion matrix [true class][pred]. **Hold 2.5+ s** → logit mean + per-trial stats (if any)
// and print confusion matrix, then **clear** trial buffer + matrix. Shield: tinyml_shield.
//
...
//   cp deploy/m3_int8_headers/m3_*.h deploy/m3_nano33_int8/
...
// Max length of one START…STOP run. Keep this conservative: DeepConvLSTM tensor allocation
// is SRAM-sensitive on Nano 33 BLE. 600 samples = 30 s @ 20 Hz (or 6 s @ 100 Hz).
const int kMaxSessionSamples = 1500;
// Cumulative logit cap (multiple STOP batches stack until full). 64*6*4 = 1.5 KB.
    ...
    if (c == '\n' || c == '\r') {
      if (slen > 0) {
        ...
        if (sbuf[0] != '\0' &&
          ...
          Serial.println(
              F("[help] commands: start, stop, toggle, avg, ping, gt <name>"));
          Serial.println(
              F("[help] GT names: Walking, Jogging, Upstairs, Downstairs, Sit, Standing  (or 0-5, w/j/u/d/si/st)"));
          Serial.println(
              F("[help] long-hold the button (or 'avg') = mean of buffered trials + confusion matrix, then clear."));
        } else if (sbuf[0] != '\0' &&
                   (strieq(sbuf, "start") || strieq(sbuf, "stop") || strieq(sbuf, "toggle"))) {
          // start / stop / toggle all do the same thing as a button click — they flip recording state.
          // (Distinct names exist so the desktop UI feels right; firmware semantics are identical.)
          action_toggle_record();
        } else if (sbuf[0] != '\0' && strieq(sbuf, "avg")) {
          action_average_buffered();
        } else if (sbuf[0] != '\0' && strieq(sbuf, "ping")) {
          // Lets the desktop controller verify a live connection on demand.
          Serial.println(F("[PONG] m3_nano_int8_serial_imu"));
          emit_state_line();
        } else if (sbuf[0] != '\0' && strncmp(sbuf, "gt ", 3) == 0) {
          // Explicit "gt <name>" form (the bare-name form below still works too).
          char work[kMaxSerialLine + 1];
          strncpy(work, sbuf + 3, kMaxSerialLine);
          ...
          if (p >= 0) {
            ...
            Serial.print(F(" ("));
            Serial.print(p);
            Serial.println(F(")  —  click START, then same as before."));
          } else {
            Serial.print(F(">>> [GT] unknown: \""));
            Serial.print(sbuf);
...
// Emit a one-line machine-readable state marker so the desktop controller can
// follow recording state without scraping every log line.
static void emit_state_line() {
  Serial.print(F("[STATE] recording="));
  Serial.println(g_recording ? F("1") : F("0"));
}

// Action helpers — invoked by both the physical button and the Serial commands.
static void action_toggle_record() {
  if (!g_recording) {
    g_session_n = 0;
    ...
    Serial.println(
        F(">>> REC — move, then click STOP (or short press+release the button) "
          "to run sliding windows. <<<"));
    emit_state_line();
  } else {
    g_recording = false;
    Serial.print(F(">>> STOP — "));
...
static void action_average_buffered() {
...
void setup() {
  ...
  Serial.println();
  Serial.println(
      F("Serial: type class name (Walking… or 0-5) to set GT, then 'start' / 'stop' / 'avg'. "
