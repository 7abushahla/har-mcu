// M3 HAR INT8 (LIVE IMU) — Nano 33 BLE + TinyML shield. Daghero QAT or DeepConvLSTM PTQ (build flags).
// Session → windowing matches offline pipeline.
//
// Features (same role as legacy deepconvlstm_nano_int8_live_imu): Serial ground-truth labels,
// START/STOP recording, sliding-window inference, per-window pred+conf on Serial, cumulative
// confusion matrix, long-hold to average buffered logits + print CM. This variant adds M3 model
// + norm headers (train z-score) and optional T=50/T=100 / Daghero / DeepConv via build flags.
//
