...
# Submit the dual-domain eval matrix for the v2 bounded-rotation run.
# Pass M3_SLURM_DEPENDENCY=afterok:<TRAIN_JOBID> when submitting after training.

export M3_DUAL_EVAL_OUTPUT_DIR="${M3_DUAL_EVAL_OUTPUT_DIR:-reports/m3/dual_domain_eval_v2_bounded20_p025}"
if [ -z "${M3_DUAL_EVAL_ON_ARTIFACT_SUFFIX:-}" ]; then
  export M3_DUAL_EVAL_ON_ARTIFACT_SUFFIX='accel_rotation_v2_bounded20_p025/{model_variant}/{experiment_code}'
fi
