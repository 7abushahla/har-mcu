...
if [ "$#" -lt 1 ]; then
  echo "Usage: bash scripts/slurm/submit_m3_build_dataset.sh configs/m3/E00_wisdm_m2_anchor.yaml [--window-size N] [--split-protocol random_stratified] [--max-windows-per-class N] [--artifact-suffix smoke]" >&2
  exit 2
fi
...
if [ "${M3_ALLOW_USER_HOLDOUT:-0}" != "1" ]; then
  if grep -q "user_holdout" "$CONFIG" || echo "$EXTRA_ARGS" | grep -q "user_holdout"; then
    echo "Refusing to submit user_holdout dataset work. Set M3_ALLOW_USER_HOLDOUT=1 only if explicitly approved." >&2
    exit 1
  fi
fi
...
PARTITION="${M3_SLURM_BUILD_PARTITION:-gpu}"
GRES="${M3_SLURM_BUILD_GRES:-gpu:1}"
CPUS="${M3_SLURM_CPUS:-4}"
MEM="${M3_SLURM_MEM:-15G}"
TIME_LIMIT="${M3_SLURM_BUILD_TIME:-12:00:00}"
...
JOB_NAME="m3build_${JOB_BASENAME}"
...
python -m src.m3.build_dataset --config "${CONFIG}" ${EXTRA_ARGS}
SBATCH
